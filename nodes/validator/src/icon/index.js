...
function subscribeNewBlock(web3, callback) {
    web3.eth.subscribe('newBlockHeaders', (err, res) => {
        if (err)
            return logger.icon.error('subscribeNewBlock subscribe error: ' + err.message);
    ...
    return new Promise((resolve, reject) => {
        ...
        for (let event of _eventList) {
            nodeConfig.contract.getPastEvents(event.name, options).then(events => {
                ...
                if (events.length > 0) {
                    logger.icon.info(`[${nodeConfig.name.toUpperCase()}] Get '${event.name}' event from block ${blockNumber}. length: ${events.length}`);
                }
...
function validateSwap(data) {
    ...
    icon.getTransactionResult(data.bytes32s[1]).then(async receipt => {
        ...
        logger.icon.error('validateSwap getReceipt call error: ' + e.message);
    });
