...
const rpcAggregator = new RPCAggregator(chainName, settings.MATIC_CHAIN_ID);
...
function initialize(_account) {
    ...
    if (govInfo.chain === chainName) {
        ...
        brittoConfig.abi = Britto.getJSONInterface({filename: 'MaticVault.abi', version: 'v2'});
    } else {
        brittoConfig.address = settings.BridgeAddress.Matic.MaticMinterContract;
        brittoConfig.abi = Britto.getJSONInterface({filename: 'MaticMinter.abi', version: 'v2'});
    }
    const rpc = settings.Endpoints.Matic.rpc;
...
async function validateSwap(data) {
    ...
    async function applyLimitation(gateKeeper, data) {
        ...
        for(var i = 0; i < validators.length; i++){
            if(validators[i].toLowerCase() === validator.address.toLowerCase()){
                logger.matic.error(`Already signed. applyLimitation: ${hash}`);
                return;
            }
        }
        ...
        let tx = await orbitHub.web3.eth.sendSignedTransaction(signedTx.rawTransaction, async (err, thash) => {
            if(err) {
                logger.matic.error(`applyLimitation error: ${err.message}`);
                return;
            }

            logger.matic.info(`applyLimitation: ${thash}`);
...
async function getBalance(tokenAddr) {
    ...
    if(tokenAddr === "0x0000000000000000000000000000000000000000"){
        amount = await mainnet.web3.eth.getBalance(govInfo.address).catch(e => {
            logger.matic.error(`${tokenAddr} getBalance error : ${e.message}`);
        });
    }
    else{
