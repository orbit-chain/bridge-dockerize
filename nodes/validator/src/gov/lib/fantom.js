...
const errmInvalidTransaction =  {
    "errm": "[FANTOM] Invalid Transaction Id",
...
async function _getTransaction(node, data, abiDecoder) {
    ...
    let myAddress = monitor.address["FANTOM"];
    if(!myAddress){
        return errmInvalidTransaction;
    }
    ...
    for (var c in config.contract){
        ...
        if(config.contract[c].toLowerCase() === transaction.destination.toLowerCase() && c.includes("FANTOM")){
...
async function _confirmTransaction(node, data) {
    ...
    async function confirm() {
        ...
        if (settings.FANTOM_CHAIN_ID) {
            txOptions.chainId = settings.FANTOM_CHAIN_ID;
        }
        ...
        let gasLimit = await contract.methods.confirmTransaction(data.transactionId).estimateGas(txOptions).catch(e => {
            logger.gov.error('[FANTOM] ConfirmTransaction estimateGas error: ' + e.message)
        });

        if (!gasLimit) {
            return {
                "errm": "[FANTOM] EstimateGas Error",
...
async function getCurrentGas() {
    return settings.FANTOM_GAS_PRICE || 10 * 10 ** 9;
}
