...
const errmInvalidTransaction =  {
    "errm": "[CELO] Invalid Transaction Id",
...
async function _getTransaction(node, data, abiDecoder) {
    ...
    let myAddress = monitor.address["CELO"];
    if(!myAddress){
        return errmInvalidTransaction;
    }
    ...
    for (var c in config.contract){
        ...
        if(config.contract[c].toLowerCase() === transaction.destination.toLowerCase() && c.includes("CELO")){
...
async function _confirmTransaction(node, data) {
    ...
    async function confirm() {
        ...
        if (settings.CELO_CHAIN_ID) {
            txOptions.chainId = settings.CELO_CHAIN_ID;
        }
        ...
        let gasLimit = await contract.methods.confirmTransaction(data.transactionId).estimateGas(txOptions).catch(e => {
            logger.gov.error('[CELO] ConfirmTransaction estimateGas error: ' + e.message)
        });

        if (!gasLimit) {
            return {
                "errm": "[CELO] EstimateGas Error",
...
async function getCurrentGas() {
    return settings.CELO_GAS_PRICE || 10 * 10 ** 9;
}
