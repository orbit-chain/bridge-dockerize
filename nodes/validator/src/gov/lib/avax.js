...
const errmInvalidTransaction =  {
    "errm": "[AVAX] Invalid Transaction Id",
...
async function _getTransaction(node, data, abiDecoder) {
    ...
    let myAddress = monitor.address["AVAX"];
    if(!myAddress){
        return errmInvalidTransaction;
    }
    ...
    for (var c in config.contract){
        ...
        if(config.contract[c].toLowerCase() === transaction.destination.toLowerCase() && c.includes("AVAX")){
...
async function _confirmTransaction(node, data) {
    ...
    async function confirm() {
        ...
        if (settings.AVAX_CHAIN_ID) {
            txOptions.chainId = settings.AVAX_CHAIN_ID;
        }
        ...
        let gasLimit = await contract.methods.confirmTransaction(data.transactionId).estimateGas(txOptions).catch(e => {
            logger.gov.error('[AVAX] ConfirmTransaction estimateGas error: ' + e.message)
        });

        if (!gasLimit) {
            return {
                "errm": "[AVAX] EstimateGas Error",
...
async function getCurrentGas() {
    return settings.AVAX_GAS_PRICE || 10 * 10 ** 9;
}
