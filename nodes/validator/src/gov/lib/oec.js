...
const errmInvalidTransaction =  {
    "errm": "[OEC] Invalid Transaction Id",
    "data": "NotFoundError: Can't find data"
}

async function init(){
    const oec = Britto.getNodeConfigBase('oec');

    if (Array.isArray(settings.Endpoints.Oec.rpc)) {
        oec.rpc = settings.Endpoints.Oec.rpc[0];
    } else {
        oec.rpc = settings.Endpoints.Oec.rpc;
    }
    oec.abi = Britto.getJSONInterface({filename: 'MessageMultiSigWallet.abi'});

    new Britto(oec, 'GOV_OEC').connectWeb3();
...
async function _getTransaction(node, data, abiDecoder) {
    ...
    let myAddress = monitor.address["OEC"];
    if(!myAddress){
        return errmInvalidTransaction;
    }
    ...
    for (var c in config.contract){
        ...
        if(config.contract[c].toLowerCase() === transaction.destination.toLowerCase() && c.includes("OEC")){
...
async function _confirmTransaction(node, data) {
    ...
    async function confirm() {
        ...
        if (settings.OEC_CHAIN_ID) {
            txOptions.chainId = settings.OEC_CHAIN_ID;
        }
        ...
        let gasLimit = await contract.methods.confirmTransaction(data.transactionId).estimateGas(txOptions).catch(e => {
            logger.gov.error('[OEC] ConfirmTransaction estimateGas error: ' + e.message)
        });

        if (!gasLimit) {
            return {
                "errm": "[OEC] EstimateGas Error",
...
async function getCurrentGas() {
    return settings.OEC_GAS_PRICE || 10 * 10 ** 9;
}
