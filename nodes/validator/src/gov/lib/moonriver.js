...
const errmInvalidTransaction =  {
    "errm": "[MOONRIVER] Invalid Transaction Id",
...
async function _getTransaction(node, data, abiDecoder) {
    ...
    let myAddress = monitor.address["MOONRIVER"];
    if(!myAddress){
        return errmInvalidTransaction;
    }
    ...
    for (var c in config.contract){
        ...
        if(config.contract[c].toLowerCase() === transaction.destination.toLowerCase() && c.includes("MOONRIVER")){
...
async function _confirmTransaction(node, data) {
    ...
    async function confirm() {
        ...
        if (settings.MOONRIVER_CHAIN_ID) {
            txOptions.chainId = settings.MOONRIVER_CHAIN_ID;
        }
        ...
        if(!web3GasPrice){
            return {
                "errm": "[MOONRIVER] getGasPrice Error",
        ...
        let gasLimit = await contract.methods.confirmTransaction(data.transactionId).estimateGas(txOptions).catch(e => {
            logger.gov.error('[MOONRIVER] ConfirmTransaction estimateGas error: ' + e.message)
        });

        if (!gasLimit) {
            return {
                "errm": "[MOONRIVER] EstimateGas Error",
...
async function getCurrentGas() {
    return settings.MOONRIVER_GAS_PRICE || 10 * 10 ** 9;
}
