...
const errmInvalidTransaction =  {
    "errm": "[HARMONY] Invalid Transaction Id",
...
async function _getTransaction(node, data, abiDecoder) {
    ...
    let myAddress = monitor.address["HARMONY"];
    if(!myAddress){
        return errmInvalidTransaction;
    }
    ...
    for (var c in config.contract){
        ...
        if(config.contract[c].toLowerCase() === transaction.destination.toLowerCase() && c.includes("HARMONY")){
...
async function _confirmTransaction(node, data) {
    ...
    async function confirm() {
        ...
        if (settings.HARMONY_CHAIN_ID) {
            txOptions.chainId = settings.HARMONY_CHAIN_ID;
        }
        ...
        if(!web3GasPrice){
            return {
                "errm": "[HARMONY] getGasPrice Error",
        ...
        let gasLimit = await contract.methods.confirmTransaction(data.transactionId).estimateGas(txOptions).catch(e => {
            logger.gov.error('[HARMONY] ConfirmTransaction estimateGas error: ' + e.message)
        });

        if (!gasLimit) {
            return {
                "errm": "[HARMONY] EstimateGas Error",
...
async function getCurrentGas() {
    return settings.HARMONY_GAS_PRICE || 10 * 10 ** 9;
}
