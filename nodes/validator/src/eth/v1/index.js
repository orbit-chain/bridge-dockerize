global.logger.eth_v1 = require('./logger');
...
function initialize(_account) {
    ...
    if(govInfo.chain === chainName){
        ...
        mainnet.abi = Britto.getJSONInterface({filename: 'EthVault.abi', version: 'v1'});
    }
    else{
        mainnet.address = config.contract.ETH_MAINNET_MINTER;
        mainnet.abi = Britto.getJSONInterface({filename: 'EthMinter.abi', version: 'v1'});
    }
    ...
    new Britto(mainnet, chainName + "_v1").connectWeb3();
...
function subscribeNewBlock(web3, callback) {
    web3.eth.subscribe('newBlockHeaders', (err, res) => {
        if (err)
            return logger.eth_v1.error('subscribeNewBlock subscribe error: ' + err.message);
    ...
    return new Promise((resolve, reject) => {
        ...
        for (let event of _eventList) {
            nodeConfig.contract.getPastEvents(event.name, options).then(events => {
                ...
                if (events.length > 0) {
                    logger.eth_v1.info(`[${nodeConfig.name.toUpperCase()}] Get '${event.name}' event from block ${blockNumber}. length: ${events.length}`);
                }
...
function validateSwap(data) {
    ...
    mainnet.web3.eth.getTransactionReceipt(data.bytes32s[1]).then(async receipt => {
        ...
        let currentBlock = await mainnet.web3.eth.getBlockNumber().catch(e => {
            logger.eth_v1.error('getBlockNumber() execute error: ' + e.message);
        });
    ...
    async function valid(data) {
        ...
        for(var i = 0; i < validators.length; i++){
            if(validators[i].toLowerCase() === validator.address.toLowerCase()){
                logger.eth_v1.error(`Already signed. validated swapHash: ${hash}`);
                return;
            }
        }
        ...
        let gasLimit = await orbitHub.contract.methods.validateSwap(...params).estimateGas(txOptions).catch(e => {
            logger.eth_v1.error('validateSwap estimateGas error: ' + e.message)
        });
