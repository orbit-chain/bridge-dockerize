...
const rpcAggregator = new RPCAggregator(chainName, settings.CELO_CHAIN_ID);
...
function initialize(_account) {
    ...
    if (govInfo.chain === chainName) {
        ...
        brittoConfig.abi = Britto.getJSONInterface({filename: 'CeloVault.abi', version: 'v2'});
    } else {
        brittoConfig.address = settings.BridgeAddress.Celo.CeloMinterContract;
        brittoConfig.abi = Britto.getJSONInterface({filename: 'CeloMinter.abi', version: 'v2'});
    }
    const rpc = settings.Endpoints.Celo.rpc;
...
async function validateSwap(data) {
    ...
    async function applyLimitation(gateKeeper, data) {
        ...
        for(var i = 0; i < validators.length; i++){
            if(validators[i].toLowerCase() === validator.address.toLowerCase()){
                logger.celo.error(`Already signed. applyLimitation: ${hash}`);
                return;
            }
        }
        ...
        let tx = await orbitHub.web3.eth.sendSignedTransaction(signedTx.rawTransaction, async (err, thash) => {
            if(err) {
                logger.celo.error(`applyLimitation error: ${err.message}`);
                return;
            }

            logger.celo.info(`applyLimitation: ${thash}`);
