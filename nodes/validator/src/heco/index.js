...
const rpcAggregator = new RPCAggregator(chainName, settings.HECO_CHAIN_ID);
...
function initialize(_account) {
    ...
    if (govInfo.chain === chainName) {
        ...
        brittoConfig.abi = Britto.getJSONInterface({filename: 'HecoVault.abi', version: 'v2'});
    } else {
        brittoConfig.address = settings.BridgeAddress.Heco.HecoMinterContract;
        brittoConfig.abi = Britto.getJSONInterface({filename: 'HecoMinter.abi', version: 'v2'});
    }
    const rpc = settings.Endpoints.Heco.rpc;
...
async function validateSwap(data) {
    ...
    async function applyLimitation(gateKeeper, data) {
        ...
        for(var i = 0; i < validators.length; i++){
            if(validators[i].toLowerCase() === validator.address.toLowerCase()){
                logger.heco.error(`Already signed. applyLimitation: ${hash}`);
                return;
            }
        }
        ...
        let tx = await orbitHub.web3.eth.sendSignedTransaction(signedTx.rawTransaction, async (err, thash) => {
            if(err) {
                logger.heco.error(`applyLimitation error: ${err.message}`);
                return;
            }

            logger.heco.info(`applyLimitation: ${thash}`);
