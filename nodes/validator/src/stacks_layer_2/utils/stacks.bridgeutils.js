...
class StacksBridgeUtils extends BridgeUtils {
  ...
  async getLatestBlock() {
    const response = await request.get({
      ...
      logger.stacks_layer_2.error(`getTransaction error. tx:${thash}, err:${e.message}`);
    });
    if (!response) {
      return;
    }
  ...
  async getBalance(address) {
    const stx = await request.get({
      ...
      logger.stacks_layer_2.error(`getBalance error. addr:${address}, err:${e.message}`);
    });
    if(!stx)
        return;
  ...
  async getNonce(address) {
    const account = await request.get({
      ...
      logger.stacks_layer_2.error(`getNonce error. addr:${address}, err:${e.message}`);
    });
    if(!account)
        return;
  ...
  async makeUnsignedSTXTokenTransfer(nonce, publicKeys, quorumCount, toAddr, memo, amount) {
    ...
    const transaction = await makeUnsignedSTXTokenTransfer({
        ...
        logger.stacks_layer_2.error(`makeTransaction error. nonce: ${nonce}, err:${e.message}`);
    });
    if(!transaction)
        return;
  ...
  async estimateFee(serializedTx) {
    const stx = await request.post({
        ...
        logger.stacks_layer_2.error(`estimateFee error. tx:${serializedTx}, err:${e.message}`);
    });
    if(!stx)
        return;
  ...
  async broadcastTransaction(transaction) {
    const res = await broadcastTransaction(transaction.serialize(), settings.Endpoints.Stacks.url).catch(e => {
        logger.stacks_layer_2.error(`broadcastTransaction fail. err: ${e.message}`);
    });
    if(!res)
      return;
  ...
  getCurrentSigHash(lastSigHash, lastSignature, fee, nonce){
    ...
    try{
        ...
        logger.stacks_layer_2.error(`getCrrentSigHash fail. err: ${e.message}`);
        return;
    };
    if(!res)
      return;
  ...
  async getSigHashPreSign(curSigHash, authType, fee, nonce){
    ...
    try{
        ...
        logger.stacks_layer_2.error(`makeSigHashPreSign fail. err: ${e.message}`);
        return;
    }
    if(!res)
      return;
