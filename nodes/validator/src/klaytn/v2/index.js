...
async function initialize(_account) {
    ...
    if(config.klaytn.KLAYTN_ISKAS){
        ...
        node.peggingType = `${chainName}_kas`;
...
async function validateSwap(data) {
    ...
    async function applyLimitation(gateKeeper, data) {
        ...
        for(var i = 0; i < validators.length; i++){
            if(validators[i].toLowerCase() === validator.address.toLowerCase()){
                logger.klaytn_v2.error(`Already signed. applyLimitation: ${hash}`);
                return;
            }
        }
        ...
        let tx = await orbitHub.web3.eth.sendSignedTransaction(signedTx.rawTransaction, async (err, thash) => {
            if(err) {
                logger.klaytn_v2.error(`applyLimitation error: ${err.message}`);
                return;
            }

            logger.klaytn_v2.info(`applyLimitation: ${thash}`);
