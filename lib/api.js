...
module.exports = {
    ...
    bible: {
        get(path, params = {}) {
            if (!VALIDATOR_MONITOR || !VALIDATOR_MONITOR.ozys || !VALIDATOR_MONITOR.ozys.bible) {
                return new Promise((_, reject) => {
                    reject(new Error(`Bible Endpoint missing.`));
                });
            }
            let bible = VALIDATOR_MONITOR.ozys.bible;
            if (bible.endsWith('/')) {
                bible = bible.substring(0, bible.length-1);
            }
            if (path.startsWith('/')) {
                path = path.substring(1);
            }
            return req.get(`${bible}/${path}`, {
                qs: params,
                headers: {
